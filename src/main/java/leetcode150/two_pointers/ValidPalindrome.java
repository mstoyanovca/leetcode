...
public class ValidPalindrome {
    ...
    public boolean isPalindromeBruteForce(String s) {
        String filtered = s.chars().mapToObj(c -> (char) c).filter(c -> Character.isAlphabetic(c) || Character.isDigit(c)).map(Character::toLowerCase).map(String::valueOf).collect(Collectors.joining());
