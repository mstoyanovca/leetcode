package leetcode150.multidimensional_dp;

public class UniquePaths2 {
    // +1 is reserved for obstacles, working with negative numbers:
    public int uniquePathsWithObstacles(int[][] obstacleGrid) {
        if (obstacleGrid[0][0] == 1) return 0;
        int m = obstacleGrid.length;
        int n = obstacleGrid[0].length;

        for (int i = 0; i < m; i++) if (obstacleGrid[i][0] != 1) obstacleGrid[i][0] = -1;
        for (int j = 0; j < n; j++) if (obstacleGrid[0][j] != 1) obstacleGrid[0][j] = -1;

        for (int i = 1; i < m; i++) {
            for (int j = 1; j < n; j++) {
                if (obstacleGrid[i][j] != 1) {
                    obstacleGrid[i][j] = (obstacleGrid[i - 1][j] != 1 ? obstacleGrid[i - 1][j] : 0) + (obstacleGrid[i][j - 1] != 1 ? obstacleGrid[i][j - 1] : 0);
                }
            }
        }

        return obstacleGrid[m - 1][n - 1] != 1 ? -obstacleGrid[m - 1][n - 1] : 0;
    }
}
