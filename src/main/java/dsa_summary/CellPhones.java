...
public class CellPhones {
    /* int[][] cellphones = new int[][]{{10, 15}, {10, 15}, {10, 15}};
       ...
       start talking on the first phone, till the battery is fully empty, plug it in to charge
       continue talking on the second, third, etc. phone, till one of the previous phones has fully charged
       return to a previous phone with the lowest index, which has fully charged
       return the index of the phones, in the order talked for total 'time' minutes
       for time = 40 minutes:
    ...
    */
    public List<Integer> talk(int[][] cellphones, int time) {
        ...
        int[] chargedAt = new int[n];
        int timestamp = 0;
        List<Integer> result = new ArrayList<>();

        while (timestamp < time) {
            for (int i = 0; i < n; i++) {
                if (chargedAt[i] <= timestamp) {
                    timestamp += cellphones[i][0];
                    chargedAt[i] = timestamp + cellphones[i][1];
                    result.add(i);
                    break;
                }
            }
